...
var cnItems = {
    ...
    'More and more treasures': '越来越多的宝藏',
    ...
    ', Treasures help you get more skills. Lol. Increases your max level by 197,260,131. Increases your skill powered gain by 310.93%. Needs 60 really last (9) quest challenge competitions for treasure prestige.': '，宝藏可帮助您获得更多技能。 哈哈。 最大等级提高197,260,131。 使您的技能获得的增益提高310.93％。 需要进行60次真正的最后（9）挑战挑战竞赛才能获得宝藏声望。',
    ...
    'Adds +N% of skill/sec, where N = log[6.18](total treasures + 1)*10': '增加 +N% 技能/秒, 当 N = log[6.18](总宝藏 + 1)*10',
    'C8/sec is multiplied by 9th milestone effect value: 1.4692119257558902e35': 'C8/秒乘以9个里程碑效应值:1.4692119257558902e35',
    'Removes ability to treasure prestige, but gain 10% of treasure value / sec': '移除宝藏声望的能力，但获得宝藏价值的10%/秒',
    "Level": "等级",
    ...
    "default": "默认",
    "damage": "伤害",
    "You can max damage prestige": "你可以最大伤害声望",
    "Max Level is doubled": "最高等级翻倍",
    "Keeps all skills milestones": "保留所有技能里程碑",
    "Keeps 2, 5, 6 and 8th skill milestones on reset, also autobuys treasure upgrades": "保持 2、5、6 和 8 个技能里程碑重置，还自动购买宝藏升级",
    "Ready for damage?": "准备好承受伤害了吗？",
    "": "",
    "": "",
    "": "",
    "": "",
...
var cnPrefix = {
    ...
    "Treasure gain is multiplied by ": "宝藏收益乘以",
    ...
    "Treasure gain is multiplied by log1000(skills + 1)+1": "宝藏收益乘以log1000(技能+1)+1",
    ...
    "Damage effects are powered to ^": "伤害效果增强至 ^",
    "Total damage: ": "总伤害: ",
    "Multiplies base Lv. gain by ": "基础等级增益乘以 ",
    "Previous milestone effect is multiplied by ": "之前的里程碑效果乘以 ",
    "Going further": "更进一步",
    "Treasure gain is powered to ^": "宝藏增益增强至 ^",
    "": "",
    "": "",
    "": "",
    "": "",
...
var cnRegReplace = new Map([
    ...
    [/^log([\d\.]+)\(treasures \+ ([\d\.]+)\)\+([\d\.]+)$/, 'log$1\(宝藏 \+ $2\)\+$3'],
    [/^\^\(([\d\.]+)\) \+ log([\d\.]+)\)\(cur. treasures \+ ([\d\.]+)\)\)$/, '\^\($1 \+ log$2\(当前宝藏 \+ $3\)'],
    [/^\^\(([\d\.]+)\)\+\(completions\/(.+)\)$/, '\^\($1\+\(已完成\/$2\)'],
    [/^([\d\.]+)\/sec$/, '$1\/秒'],
    [/^([\d\.]+) level$/, '$1 等级'],
    [/^It\'ll be fun \(([\d\.,]+) damage$/, '这会很有趣（$1 伤害'],
    [/^Save automation \(([\d\.,]+) damage$/, '保存自动化（$1 伤害'],
    [/^Treasure damager \(([\d\.,]+) damage$/, '宝藏破坏者（$1 伤害'],
    [/^Speedrun \(([\d\.,]+) damage$/, '速通（$1 伤害'],
    [/^Ultimate damage \(([\d\.,]+) damage$/, '终极伤害（$1 伤害'],
    [/^Faster Start \(([\d\.,]+) damage$/, '更快开始（$1 伤害'],
    [/^Double Level \(([\d\.,]+) damage$/, '双倍伤害（$1 伤害'],
    [/^([\d\.,]+) exp$/, '$1 经验值'],
    [/^([\d\.,]+) rubies$/, '$1 红宝石'],
    [/^([\d\.,]+) experience$/, '$1 经验值'],
    [/^e([\d\.,]+) loot$/, 'e$1 战利品'],
    [/^e([\d\.,]+) experience$/, 'e$1 经验值'],
    [/^ee([\d\.,]+) experience$/, 'ee$1 经验值'],
    [/^ee([\d\.]+)e([\d\.,]+) experience$/, 'ee$1e$2 经验值'],
    [/^ee([\d\.]+)e([\d\.,]+) gold$/, 'ee$1e$2 黄金'],
    [/^([\d\.,]+) quests$/, '$1 任务'],
    [/^e([\d\.,]+) quests$/, 'e$1 任务'],
    [/^([\d\.,]+) treasures$/, '$1 宝藏'],
    [/^e([\d\.,]+) treasures$/, 'e$1 宝藏'],
    [/^([\d\.,]+) level$/, '$1 等级'],
    [/^([\d\.,]+) loot$/, '$1 战利品'],
    [/^([\d\.,]+) skills$/, '$1 技能'],
    [/^([\d\.,]+) damage$/, '$1 伤害'],
    ...
    [/^e([\d\.]+)e([\d\.,]+) quests$/, 'e$1e$2 任务'],
    [/^([\d\.]+)e([\d\.,]+) treasures$/, '$1e$2 宝藏'],
    [/^e([\d\.]+)e([\d\.,]+) treasures$/, 'e$1e$2 宝藏'],
    [/^([\d\.]+)e([\d\.,]+) rubies$/, '$1e$2 红宝石'],
    [/^e([\d\.]+)e([\d\.,]+) rubies$/, 'e$1e$2 红宝石'],
    [/^([\d\.]+)e([\d\.,]+) level$/, '$1e$2 等级'],
    [/^e([\d\.]+)e([\d\.,]+) level$/, 'e$1e$2 等级'],
    [/^([\d\.]+)e([\d\.,]+) skills$/, '$1e$2 技能'],
    [/^e([\d\.]+)e([\d\.,]+) skills$/, 'e$1e$2 技能'],
    [/^([\d\.]+)e([\d\.,]+) exp$/, '$1e$2 经验值'],
    [/^([\d\.]+)e([\d\.,]+) experience$/, '$1e$2 经验值'],
    [/^e([\d\.]+)e([\d\.,]+) experience$/, 'e$1e$2 经验值'],
    [/^([\d\.]+)e([\d\.,]+) loot$/, '$1e$2 战利品'],
    [/^e([\d\.]+)e([\d\.,]+) loot$/, 'e$1e$2 战利品'],
    [/^([\d\.]+)e([\d\.,]+) gold$/, '$1e$2 黄金'],
    [/^e([\d\.]+)e([\d\.,]+) gold$/, 'e$1e$2 黄金'],
    ...
    [/^Upgrade 3,1 power goes from (.+)\/(.+) to (.+)\/(.+). Unlocks new rows of xp and gold upgrades.$/, '升级3,1的效果从$1\/$2增加到$3\/$4。解锁新一行的经验和黄金升级。'],
    ...
    [/^Cost\: (.+) treasures\n\t\t$/, '成本\: $1 宝藏\n\t\t'],
    [/^(.+) \/ (.+) exp\t\t\n\t\t$/, ' $1 \/ $2 经验值\t\t\n\t\t'],
    [/^(.+) \+ floor\(\(skills\/(.+)\)\^(.+)\) = (.+)$/, '$1 \+ floor\(\(技能\/$2\)\^$3\) = $4'],
    [/^Autobuy up to (.+) passive upgrades\/tick$/, '自动购买高达 $1 被动升级\/tick'],
    [/^Damage helps you get more levels not based on your challenge: (.+). Treasure gain: (.+)$/, '伤害可以帮助您获得更多等级，而不是基于您的挑战：$1。 宝藏增益：$2'],
    [/^Get (.+) max rubies$/, '获得 $1 个最大红宝石'],
    [/^Get (.+) quests$/, '获得 $1 个任务'],
    [/^Get faster treasures \(Get (.+) skills$/, '获得更快的宝藏（获得 $1 技能'],
    [/^Get (.+) skills$/, '获得 $1 技能'],
    [/^Get (.+) skill$/, '获得 $1 技能'],
    [/^Total treasures help you get more skills. Lol. Increases your max level by (.+). Increases your skill powered gain by (.+). Needs 60 really last (.+) quest challenge completions for treasure prestige.$/, '总计宝藏可以帮助你获得更多技能。Lol.使你的最高等级增加 $1。增加你的技能增益 $2。需要60次真正的最后 $3 任务挑战完成才能获得宝藏声望。'],
    [/^Skills can make you level up and complete first two layers faster. Total S. divides loot, quests and ruby exp by (.+). Multiplies XP and gold exponents by (.+). Needs 100 last (.+) quest challenge competitions for skill prestige.$/, '技能可以让你更快地升级并完成前两层。总计技能将战利品、任务和红宝石经验值除以 $1。将经验值和黄金指数乘以 $2。需要100次最后 $3 任务挑战赛才能获得技能声望。'],
    ...
    [/^Removes ability to treasure prestige, but gain (.+) of treasure value \/ sec$/, '移除宝藏声望的能力，但每秒获得 $1 的宝藏价值'],
    ...
    [/^You are gaining (.+) treasures per second$/, '您每秒获得 $1 宝藏'],
